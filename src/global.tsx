...
// 启用兼容模式 https://immerjs.github.io/immer/docs/installation
enableAllPlugins();
...
/**
 * @see https://umijs.org/zh/guide/app-structure.html#src-global-js-ts
 */

/**
 * 创建一个便捷 dispatch 的函数
 */
window.$fastDispatch = (action, payload = {} as any) => {
  let type = action;

  if (action && typeof action === 'function') {
    type = action(window.$modelEffectsAndReducers);
  }

  // // dispatch 的传参不一定要限制在 payload 对象里面的
  // eslint-disable-next-line no-underscore-dangle
  return window.__AppStore.dispatch({ type, payload } as any) as Promise<any>;
};

/**
 * 包装一下 Promise
 *
 * @example const { result, error } = await window.$wrapperPromise(promiseFunc);
 */
window.$wrapperPromise = <T, ERR>(promise: Promise<T>): Promise<{ result: T; error: ERR | null }> =>
  promise
    .then((result) => ({ result, error: null }))
    .catch((error) => ({ result: null, error })) as any;

if (
  ['127.0.0.1', 'localhost', '193.112.84.134'].some(
    (item) => window.location.host.indexOf(item) === 0,
  )
) {
